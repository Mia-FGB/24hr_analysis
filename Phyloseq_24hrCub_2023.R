...
#OTU table 
otu <- read.csv("taxa_counts/2023_data/marti_assigned_read_count_24hrCub.csv", check.names = FALSE)
...
#Renaming those found in the 4h & 6h dataset
...
#Renaming those found in the 12h dataset
unique(samp_phylum_filter$Phylum)
...
# # Order levels of 'Phylum' based on abundance for 4/6
...
samp_phylum_filter$Phylum <- factor(
  samp_phylum_filter$Phylum,
  levels = c("Oomycota (Oomycete)", "Pseudomonadota (Bacteria)", "Actinomycetota (Bacteria)",
             "Streptophyta (Plant)", "Basidiomycota (Fungi)", "Ascomycota (Fungi)", 
             "Unassigned (NA)"))
...
# org_phy_col <- c('#AAAA00', '#77AADD', '#BBCC33', '#EE8866', '#EEDD88',
#                  '#FFAABB', '#99DDFF', '#44BB99', '#DDDDDD')

org_phy_col <- c('#77AADD', '#BBCC33', '#EE8866', '#EEDD88',
                 '#FFAABB', '#99DDFF', '#DDDDDD')
...
phylum_filter_stacked_bar <- ggplot(
  ...
  facet_wrap(~Time_hrs, scales = "free_x", ncol = 1,
            ...
            labeller = labeller(Time_hrs = c("24" = "24hr Sample", "12" = "12hr Samples")
                                 ))
...
ggsave("Graphs/clear_phylum_stacked_bar_12_24.svg", plot = phylum_filter_stacked_bar, device = "svg", bg = "transparent")
